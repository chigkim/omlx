...
class TestProcessChatMessages:
    ...
    async def test_mimo_video_preflight_expands_frames_before_validation(
        self, mock_expand_video, mock_extract, mock_extract_images
    ):
        ...
        mock_extract.return_value = (text_messages, [frame], [], [])
        mock_extract_images.return_value = (text_messages, [frame], [])
        ...
        mock_expand_video.assert_called_once_with(messages)
        mock_extract.assert_called_once_with(expanded, materialize_videos=False)

        mock_expand_video.reset_mock()
        assert engine.count_chat_tokens(messages) == 1
        mock_expand_video.assert_called_once_with(messages)
        mock_extract_images.assert_called_once_with(expanded)
